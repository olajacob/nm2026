...
3. If the task gives **startDate** (employment): **POST `/employee/employment`** with **`employee: {id}`**, **`startDate`**, **`isMainEmployer: true`**, **`taxDeductionCode`**: `"loennFraHovedarbeidsgiver"` (adjust per Swagger if needed).

### POST /customer (customer or supplier)

- Body shape: `name`, `isCustomer: true` (customers) **or** `isSupplier: true` (suppliers), **`email`** and **`organizationNumber`** whenever the prompt mentions them, **`phoneNumber`** if mentioned.
...
- **Register payment**: **GET** `/invoice` (date range + **`fields=id,invoiceNumber,invoiceDate,amountExcludingVat`**) → id; **GET** `/invoice/paymentType?fields=id` → **`paymentTypeId`**; **one** **PUT** `/invoice/{id}/:payment` via **`tripletex_put_action`** with **`paymentDate`**, **`paymentTypeId`**, **`paidAmount`** (incl. VAT). **Never** pay twice.
...
- **Before `POST /salary/transaction`:** ensure **`dateOfBirth`** / employment as above; **`GET /employee/employment?employeeId={id}&fields=id,startDate,division`**. If no employment, **`POST /employee/employment`** with **`startDate`** (e.g. **`2026-01-01`** if unspecified), **`isMainEmployer: true`**, **`taxDeductionCode`**: **`loennFraHovedarbeidsgiver`**. The agent **`execute_tool`** adds **`division: {id: 1}`** on that **POST** when missing — **virksomhet** is often set **at create** (later **`PUT`** division may return **422** *«Virksomheten kan ikke endres»*). **`PUT`** **`division`** only when still **null** and the API allows it. If **`POST /salary/transaction`** returns **«ikke knyttet mot en virksomhet»**, further **`PUT`** **`division`** for that employment is **blocked** for the rest of the **`/solve`**.
...
- **Two-step flow + ledger finalize** (implemented by **`tripletex_post_voucher`** in `agent.py` — **always** use this tool for journals; **`tripletex_post`** on **`/ledger/voucher`** is rejected):
  1. **POST `/ledger/voucher?sendToLedger=false`** with **`date`**, **`description`**, **`postings: []`**. **Do not** use **`sendToLedger=true`** on this request — empty shell → **422** *«Et bilag kan ikke registreres uten posteringer»* (OpenAPI may default **`sendToLedger`** to true if the query is omitted).
  2. For each line: **POST `/ledger/voucher/{id}/postings`** with one **Posting** JSON object (**`row`**, **`account`**, **`amountGross`**, …). On **422**, the tool **logs the error** and **retries** with **negated** **`amountGross`** once.
  3. To post the voucher to the ledger: **PUT `/ledger/voucher/{id}/:sendToLedger`** (optional query **`version`** from **GET** the voucher) — the tool runs this when **`send_to_ledger`** is true.
...
- **GET `/ledger/voucher`**: **`dateFrom`** + **`dateTo`** required; **`dateTo`** strictly after **`dateFrom`**.
...
- **`tripletex_get` / `tripletex_post` / `tripletex_put` / `tripletex_delete`**, **`tripletex_put_action`**, **`tripletex_post_voucher`** (ledger journal two-step only).
